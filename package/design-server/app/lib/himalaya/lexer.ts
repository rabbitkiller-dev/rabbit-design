...
export function makeInitialPosition(): Position {
  return {
    index: 0,
    column: 0,
    line: 0,
...
export function copyPosition(position): Position {
  return {
...
export interface StartToEndPostion { start: Position; end: Position;}
...
export default function lexer(str: string, options: ParseDefaults): Token[] {
  const state: State = {
    str,
    options,
...
export function findTextEnd(str, index): number {
  while (true) {
...
export function lexText(state: State) {
  ...
  state.tokens.push({type, content, position: {start, end}, close: true});
}
